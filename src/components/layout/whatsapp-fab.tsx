"use client";

import { usePathname } from "next/navigation";

import { siteConfig } from "@/config/site";
import { toWhatsAppNumber } from "@/lib/format";

const defaultText = `Halo ${siteConfig.brandName}, saya ingin bertanya tentang sewa mobil listrik.`;

/**
 * Floating WhatsApp shortcut.
 *
 * Hidden on the booking flow: that page has its own WhatsApp call to action
 * (including a sticky bar on mobile) and two competing green buttons in the
 * same corner would collide.
 */
export function WhatsAppFab() {
  const pathname = usePathname();
  if (pathname.startsWith("/sewa-mobil")) return null;

  const href = `https://wa.me/${toWhatsAppNumber(siteConfig.contact.whatsapp)}?text=${encodeURIComponent(defaultText)}`;

  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Hubungi kami via WhatsApp"
      className="fixed bottom-5 right-5 z-30 inline-flex size-13 items-center justify-center rounded-full bg-[#1EBE5D] text-white shadow-lift transition-transform duration-200 ease-[var(--ease-out-soft)] hover:scale-105 active:scale-95 print:hidden"
    >
      <svg viewBox="0 0 24 24" className="size-6.5" fill="currentColor" aria-hidden="true">
        <path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.87 9.87 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0 0 12.04 2Zm0 18.13h-.01a8.2 8.2 0 0 1-4.19-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.18 8.18 0 0 1-1.26-4.36c0-4.54 3.7-8.23 8.25-8.23 2.2 0 4.27.86 5.83 2.42a8.18 8.18 0 0 1 2.41 5.82c0 4.54-3.7 8.21-8.24 8.21Zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.25-.64.8-.79.97-.14.16-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.24-1.47-1.38-1.72-.15-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.13-.15.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.13-.56-1.35-.77-1.85-.2-.48-.41-.42-.56-.43h-.48c-.16 0-.43.06-.65.31-.23.25-.86.84-.86 2.05s.88 2.38 1 2.54c.12.17 1.73 2.63 4.18 3.69.58.25 1.04.4 1.4.51.59.19 1.12.16 1.55.1.47-.07 1.47-.6 1.67-1.18.21-.58.21-1.07.15-1.18-.06-.11-.23-.17-.48-.29Z" />
      </svg>
    </a>
  );
}
